...
const EditSubmissionModal = ({ submission, isOpen, onClose }: EditSubmissionModalProps) => {
  const dispatch = useDispatch();
  const { user } = useSelector((state: RootState) => state.auth);
  
  const [formData, setFormData] = useState({
    purchase_order_number: submission.purchase_order_number || '',
    has_invoice: submission.document_type === 'factura',
    invoice_number: submission.invoice_number || '',
    invoice_date: submission.invoice_date,
    client_payment_date: submission.client_payment_date,
    client_name: submission.client_name,
    amount_with_iva: submission.amount_with_iva?.toString() || '',
    amount_without_iva: submission.amount_without_iva?.toString() || '',
    is_mercado_libre_sale: submission.is_mercado_libre_sale || false,
    ml_fee_cargos_venta: submission.ml_fee_cargos_venta?.toString() || '',
    ml_fee_envios: submission.ml_fee_envios?.toString() || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedSubmission: CommissionSubmission = {
        ...
        invoice_pdf_path: file ? `/uploads/${file.name}` : submission.invoice_pdf_path,
      ...
      setError(err.response?.data?.detail || 'Error al actualizar la solicitud');
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div className="fixed inset-0 transition-opacity bg-secondary-500 bg-opacity-75" onClick={onClose}></div>
        ...
        <div className="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-xl">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-medium text-secondary-900">Editar Solicitud de Comisión</h3>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="mt-3 p-3 rounded-lg border">
                <div className="text-sm space-y-1">
                  <div className="flex justify-between">
                    <span className="text-secondary-700">Base para comisión:</span>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700 mt-1">
                        Ingrese las comisiones que Mercado Libre descontó de esta venta.
                      </p>
                    </div>
                  </div>
            ...
            <div>
              <label htmlFor="document_pdf" className="block text-sm font-semibold text-secondary-900 mb-2">
                Actualizar {formData.has_invoice ? 'Factura' : 'Orden de Compra'} PDF (opcional)
              </label>
              <input
                type="file"
                name="document_pdf"
              ...
              <p className="mt-2 text-xs text-secondary-500">
                {file ? `Nuevo archivo: ${file.name}` : `Archivo actual: ${submission.invoice_pdf_path?.split('/').pop()}`}
              </p>
            </div>
